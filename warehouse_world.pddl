...
(:action putdownPallette
	:parameters (?r - robot ?l - location ?p - pallette)
	:precondition (and(has ?r ?p)(not (free ?r))(at ?r ?l)(at ?p ?l))
	:effect (and(free ?r)(not (has ?r ?p)))
)

(:action robotMoveWithoutPallette
	:parameters (?r - robot ?loc1 - location ?loc2 - location)
	:precondition (and(free ?r)(at ?r ?loc1)(no-robot ?loc2)(connected ?loc1 ?loc2))
	:effect (and(not (at ?r ?loc1))(not (no-robot ?loc2))(at ?r ?loc2)(no-robot ?loc1))
)

(:action robotMoveWithPallette
	:parameters (?r - robot ?loc1 - location ?loc2 - location ?p - pallette)
	:precondition (and(has ?r ?p)(not (free ?r))(at ?r ?loc1)(at ?p ?loc1)(no-robot ?loc2)(no-pallette ?loc2)(connected ?loc1 ?loc2))
	:effect (and(not (at ?r ?loc1))(not (at ?p ?loc1))(at ?r ?loc2)(at ?p ?loc2)(not (no-robot ?loc2))(not (no-pallette ?loc2))(no-robot ?loc1)(no-pallette ?loc1))
)
   
   
(:action moveItemFromPalletteToShipment
	:parameters (?p - pallette ?s - shipment ?o - order ?si - saleitem ?l - location)
	:precondition (and(ships ?s ?o)(orders ?o ?si)(contains ?p ?si)(packing-at ?s ?l)(started ?s)(at ?p ?l))
...
(:action completeShipment
	...
	:precondition (and(ships ?s ?o)(orders ?o ?si)(includes ?s ?si))
